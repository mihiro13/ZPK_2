...
export function displayLabels(player) {
    ...
    const display = initializeDisplay(labels, { digit: digit, separate: separate, color1: color1, color2: color2 });
...
function initializeDisplay(labels, decorateOptions) {
    if (!labels) return;
    ...
    display.yaw = formatNumberLabel('F', labels.yaw, labelCharacters.yaw);
